...
export default function CameraScene(){
 ...
 const [assignments,setAssignments]=useState<ExpressionAssignments>(preferences.assignments);
 ...
 const [exactPairs,setExactPairs]=useState<string[]|null>(()=>!requested&&!requestedMeme?preferences.exactPairs??null:null);
 const [enabled,setEnabledState]=useState<string[]>(()=>!requested&&!requestedMeme&&preferences.enabled?preferences.enabled:memes.filter(m=>requestedMeme?m.id===requestedMeme:requested==='all'?true:requested==='calls'?callIds.includes(m.id):requested?inPack(m,requested as PackId):(broadcast||new URLSearchParams(location.search).has('send')?callIds:coreIds).includes(m.id)).map(m=>m.id));
