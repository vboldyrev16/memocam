...
export function MemeSet({authorCount,onAuthor,selectedOnly=false,onBrowse,favorite,onSaveFavorite,onLoadFavorite,faceChecks,handsClear,memes,assignments,keys,onChange,onPreset,latest,stream,tracking,progress,trials,peekState,saved,calibrated,onRecalibrate}:{authorCount:number;onAuthor:()=>void;selectedOnly?:boolean;onBrowse:()=>void;favorite:FavoriteSet|undefined;onSaveFavorite:()=>void;onLoadFavorite:()=>void;faceChecks:ReturnType<typeof expressionChecks>;handsClear:boolean;memes:Meme[];assignments:ExpressionAssignments;keys:string[];onChange:(keys:string[])=>void;onPreset:()=>void;latest:RefObject<LatestObservation>;stream:MediaStream|null;tracking:string;progress:number;trials:Trial[];peekState:{face:boolean;hand:boolean;mouth:boolean;fingers:boolean;cheek:boolean};saved:boolean;calibrated:boolean;onRecalibrate:()=>void}){
 ...
 return <section className="meme-set" aria-label="Точный набор реакций">
  ...
  <div className="set-toolbar">{authorCount>0&&<button onClick={onAuthor}>Подборка автора · {authorCount}</button>}{selectedOnly&&<button onClick={onBrowse}>Добавить из полного списка</button>}<strong>{validKeys.length} действий включено</strong><button onClick={()=>onChange([])}>Снять все действия</button>{!selectedOnly&&<button onClick={onPreset}>Окак + Тиньков</button>}<button onClick={exportList}>Скачать полный список</button></div>
